...
const Header = ({ darkMode, user, iconColor, toggleDarkMode, showBackButton, onBackClick }) => {
  return (
    <div className="relative h-24 bg-gray-500/30 flex items-center px-4">
      ...
      <Link to="/">
        <button
          className="absolute top-3 right-3 text-2xl rounded-full p-1 hover:bg-gray-600 transition"
          aria-label="Close"
        >
      ...
      {/* Avatar + Info */}
      <div
        className={`w-14 h-14 border-2 rounded-full overflow-hidden ${
          darkMode ? "border-slate-500 bg-white" : "border-slate-400 bg-slate-200"
        }`}
      >
        {user.photo ? (
      ...
      <div className="leading-tight pl-2">
        <h1 className="text-xl font-semibold text-white capitalize">{user.name}</h1>
